...
import { discoverOwnedSsus, getStoredActiveSsu, setStoredActiveSsu, clearStoredActiveSsu, type DiscoveredSsu } from "@/lib/chain/ssuDiscovery";
...
export default function Home() {
  ...
  // SSU resolution order:
  //   1. dapp-kit assembly.id   (game passed ?itemId= and resolved it)
  //   2. ?ssu= URL param        (manual override, also generated by admin page)
  //   3. localStorage           (last picked SSU)
  //   4. Auto-discovery         (player's owned SSUs with slots authorised)
  const [discoveredSsus, setDiscoveredSsus] = useState<DiscoveredSsu[]>([]);
  const [discovering, setDiscovering] = useState(false);
  const [storedSsu, setStoredSsu] = useState<string | null>(null);

  // Load stored SSU once on mount
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setStoredSsu(getStoredActiveSsu());
  }, []);

  const urlSsu = assembly?.id ?? searchParams.get("ssu") ?? "";
  const autoSsu = discoveredSsus.find((s) => s.slotsAuthorized)?.ssuId ?? discoveredSsus[0]?.ssuId ?? "";
  const ssuId = urlSsu || storedSsu || autoSsu || "";

  function pickSsu(id: string) {
    setStoredActiveSsu(id);
    setStoredSsu(id);
  }

  // Run discovery when character is known and no SSU is yet resolved
  useEffect(() => {
    if (!character?.characterId) return;
    if (urlSsu || storedSsu) return;
    if (discovering) return;
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setDiscovering(true);
    discoverOwnedSsus(character.characterId)
      .then(setDiscoveredSsus)
      .catch((e) => { console.error("SSU discovery failed:", e); setDiscoveredSsus([]); })
      .finally(() => setDiscovering(false));
  }, [character?.characterId, urlSsu, storedSsu]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  }, [assembly?.id, assemblyLoading, assemblyError, urlSsu, storedSsu, autoSsu, ssuId]);
  ...
  return (
    ...
    >
      ...
      {isChainConfigured && isConnected && !ssuId && (
        <div style={{ width: "100%", maxWidth: 610, marginTop: 8, padding: "10px 14px",
          background: "rgba(229,53,53,0.06)", border: "1px solid rgba(229,53,53,0.35)",
          color: "var(--red)", fontFamily: "var(--font-mono)", fontSize: 11,
        }}>
          {assemblyLoading || discovering
            ? "Discovering active SSU…"
            : discoveredSsus.length === 0
            ? assemblyError
              ? `Could not load SSU from game: ${assemblyError}`
              : "No SSU detected. Configure the SSU's custom-dapp URL to include ?ssu=<sui_object_id> (see admin page)."
            : (
              <div>
                <p style={{ marginBottom: 6 }}>Pick the SSU you want to play on:</p>
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  {discoveredSsus.map((s) => (
                    <button
                      key={s.ssuId}
                      onClick={() => pickSsu(s.ssuId)}
                      className="btn-ghost"
                      style={{
                        textAlign: "left",
                        ...
                        fontSize: 11,
                        padding: "4px 8px",
                        borderColor: s.slotsAuthorized ? "var(--teal)" : "var(--teal-dim)",
                        color: s.slotsAuthorized ? "var(--teal)" : "var(--teal-dim)",
                      }}
                    >
                      {s.slotsAuthorized ? "✓" : "·"} {s.ssuId.slice(0, 10)}…{s.ssuId.slice(-6)}
                      {!s.slotsAuthorized && <span style={{ marginLeft: 8, opacity: 0.7 }}>(not authorised)</span>}
                    </button>
                  ))}
                </div>
              </div>
      ...
      {/* Active SSU badge — small, clickable to clear (so user can re-pick) */}
      {isChainConfigured && isConnected && ssuId && (
        <div style={{ width: "100%", maxWidth: 610, marginTop: 6, display: "flex",
          alignItems: "center", justifyContent: "space-between",
          fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--teal-dim)",
        }}>
          <span>SSU: {ssuId.slice(0, 10)}…{ssuId.slice(-6)}</span>
          {(storedSsu === ssuId) && (
            <button
              onClick={() => { clearStoredActiveSsu(); setStoredSsu(null); setDiscoveredSsus([]); }}
              className="btn-ghost"
              style={{ fontSize: 9, padding: "2px 6px", borderColor: "var(--teal-dim)", color: "var(--teal-dim)" }}
            >
              Change SSU
            </button>
          )}
