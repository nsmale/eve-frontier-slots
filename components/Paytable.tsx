...
export default function Paytable() {
  ...
  return (
    /* position: relative so the panel can anchor absolutely to this container */
    <div style={{ position: "relative" }}>
      <button onClick={() => setOpen(v => !v)} className="btn-ghost" style={{ fontSize: 11 }}>
        {open ? "▲" : "▼"} Paytable
      </button>

      {/* Panel anchors below the button without affecting parent layout */}
      <div style={{ position: "absolute", bottom: "calc(100% + 8px)", right: 0, zIndex: 50, width: "min(480px, 90vw)" }}>
        <AnimatePresence>
          {open && (
            <motion.div
